 ...
 * \brief   Implementation of fragment shader for the room textures.
 */
//----------------------------------------------------------------------------------------
...
uniform sampler2D ceilingSampler;
...
void main() {
    ...
    if (abs(norm.y) > 0.5) {
        if (norm.y > 0.0)
            texColor = vec3(texture(texSampler, texCoord_v * 3.0));
        else
            texColor = vec3(texture(ceilingSampler, texCoord_v * 2.0));
    } else {
        texColor = vec3(texture(wallSampler, texCoord_v * 2.0));
    }
